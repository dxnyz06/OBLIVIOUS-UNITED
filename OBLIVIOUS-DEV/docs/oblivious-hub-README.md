...
.NET hub. The legacy `AiMindBridge/` folder is decommissioned and only
...
# Edit .env: set at least one *_API_KEY and KEYVAULT_PASSPHRASE
npm install
npm start
```

The first run creates `keyvault.dat` next to the executable and seeds
it from the `*_API_KEY` environment variables. Subsequent runs read
keys from the encrypted vault using `KEYVAULT_PASSPHRASE`.
