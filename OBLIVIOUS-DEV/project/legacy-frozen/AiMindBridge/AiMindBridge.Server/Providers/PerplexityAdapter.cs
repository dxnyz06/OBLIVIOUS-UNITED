...
public sealed class PerplexityAdapter : ProviderAdapterBase
{
    public override string Name => "perplexity";
    private const string DefaultModel = "llama-3.1-sonar-small-128k-online";
    private const string ApiUrl = "https://api.perplexity.ai/chat/completions";

    public PerplexityAdapter(ProviderSettings settings, ILogger<PerplexityAdapter> logger)
    ...
    protected override object BuildHttpPayload(string prompt, string apiKey)
    {
        ...
        return new
        {
            ...
            messages    = new[]
            {
                new { role = "system", content = "You are a professional forex trade planner with web-aware macro reasoning. Always respond only with valid JSON." },
    ...
    protected override async Task<string> SendHttpAsync(object payload, CancellationToken ct)
    {
        ...
        var resp    = await Http.PostAsync(ApiUrl, content, ct);
